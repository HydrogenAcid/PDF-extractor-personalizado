...
def extract_text_pymupdf_blocks(pdf_path: str) -> List[str]:
    ...
    try:
        ...
        for page in doc:
            ...
            text = "\n".join(
                (b[4] or "").strip() for b in blocks if (b[4] or "").strip()
            )
            pages.append(text)
        doc.close()
...
def build_page_cooccurrence_graph(
    token_pages: List[List[str]], span: int = 1, min_freq: int = 2, max_vocab: int = 400
...
def top_subgraph_for_display(
    G: nx.Graph, max_nodes: int = 250, max_edges: int = 400
) -> Dict:
    if G.number_of_nodes() == 0:
        return {"nodes": [], "edges": []}

    ranked_nodes = sorted(
        G.nodes(data=True), key=lambda x: (-x[1].get("freq", 0), x[0])
    )[:max_nodes]
    ...
    for u, v, w in edges:
        edge_payload.append(
            {"from": u, "to": v, "value": int(w), "title": f"{u} — {v} (w={w})"}
        )
...
def register_graph_text(app):
    ...
    def process_graph_text():
        ...
        ocr_map = {"es": "spa", "en": "eng", "fr": "fra", "de": "deu"}
        ...
        try:
            ...
            if tokens_filtered < 10:
                return (
                    jsonify({"error": "Texto insuficiente tras quitar stopwords"}),
                    400,
                )
            ...
            return jsonify(
                {
                    ...
                    "display_graph": top_subgraph_for_display(
                        G, max_nodes=35, max_edges=80
                    ),
                }
            )
        finally:
